  ...
  app.get(
    ...
    async (req, res) => {
      ...
      const formResults = await db.query(
        `SELECT f.*, CASE WHEN (SELECT id FROM form_published WHERE form_id = f.id) IS NULL THEN 0 ELSE id END AS published_id, ( SELECT COUNT(*) FROM submission WHERE form_id = f.id ) as responseCount, ( SELECT COUNT(*) FROM submission WHERE form_id = \`form\`.\`id\` AND \`read\` = 0 ) as unreadCount FROM form AS f WHERE f.user_id = ? AND deleted_at IS NULL`,
        [user_id]
      )
