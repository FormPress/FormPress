...
class Roles extends Component {
  ...
  handleSelectRole(roleId, incMessage='') {
    ...
    Object.keys(newPermissions).forEach( (element) => {
      if (element === 'formLimit' || element === 'submissionLimit' || element === 'uploadLimit') {
        newPermissions[element] = rolePermissions[element] || "1"
      } else {
        newPermissions[element] = rolePermissions[element] || false
      }
    })
    this.setState({selectedRoleName: roleName, selectedRoleId: role_id, permissions:newPermissions, message: incMessage, saved:true})
  }
  ...
  handleFieldChange(elem, e) {
    ...
    if (elem.type === 'Checkbox') {
      ...
      const elementType = textMapArray.filter( element => element[1] === elementText)[0][0] 
      ...
      <div>
        <ul>
          {loaded &&
            data.map((role) => (
              <li
                className={this.state.selectedRoleId===role.id?"selectedrole":"role"}
                key={role.id}
                value={role.id}
          ...
          <li
            className={this.state.selectedRoleId===0?"selectedrole":"role"}
            key={0}
            value={0}
